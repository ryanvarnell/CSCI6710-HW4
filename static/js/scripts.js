  ...
  function createSlideshow(slideshowContainer, products) {
    ...
    function updateTopSlideshow(tops) {
      const slideshow = document.querySelector(
        ".slideshow.tops .slideshow-container"
      );
      slideshow.innerHTML = "";
      tops.forEach((top) => {
        const slide = document.createElement("div");
    ...
    function updateBottomSlideshow(bottoms) {
      const slideshow = document.querySelector(
        ".slideshow.bottoms .slideshow-container"
      );
      slideshow.innerHTML = "";
    ...
    filterSubmitButton.addEventListener("click", (event) => {
      ...
      fetch("/filter", {
        ...
        .then((data) => {
          ...
          const filteredBottoms = data.filter(
            (product) => product[2] === "bottom"
          );
        ...
        $.ajax({
          ...
          success: function (data) {
            window.location.href =
              "{{ url_for(templates,     filename=/login.html) }}";
          },
      ...
      if (username === "" || password === "") {
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data === "success") {
              window.location.href =
                "{{ url_for(templates,     filename=/user.html) }}";
            } else {
